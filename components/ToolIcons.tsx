...
/**
 * Optional explicit colours.
 *
 * The web uses `currentColor` plus the .ti-* classes so hover lives in CSS.
 * Satori (Next's ImageResponse, used for the OG cards) reads only inline
 * attributes — it ignores classes, custom properties and currentColor — so the
 * OG renderer passes real hex values through `tone` instead.
 */
export interface IconTone {
  ...
  /** Gold accent colour. */
  accent: string
  /** Fill behind waypoint rings — should match the surface they sit on. */
  nodeFill: string
}

const accent = (t?: IconTone) =>
  t ? { stroke: t.accent } : { className: 'ti-accent' }
const accentSolid = (t?: IconTone) =>
  t ? { stroke: t.accent, fill: t.accent } : { className: 'ti-accent-solid' }
const node = (t?: IconTone) =>
  t ? { fill: t.nodeFill, stroke: t.base } : { className: 'ti-node' }
...
const ICONS: Record<ToolIconName, (t?: IconTone) => React.ReactNode> = {
  ...
  contracting: (t) => (
    <g>
      ...
      <path {...accentSolid(t)} d="m16.1 12.6.6 1.4 1.4.6-1.4.6-.6 1.4-.6-1.4-1.4-.6 1.4-.6z" />
    </g>
  ),
  ...
  combinedRating: (t) => (
    <g>
      ...
      <path {...accent(t)} d="m14.1 8.2-4.2 7.4" />
    </g>
  ),
  ...
  retirement: (t) => (
    <g>
      ...
      <path {...accentSolid(t)} d="M16.7 5.3 21 7.1l-4.3 1.8z" />
    </g>
  ),
  ...
  mortgage: (t) => (
    <g>
      ...
      <path {...accent(t)} d="M7.9 12.4c2.2 3.5 4.7 4.7 8.3 5" />
    </g>
  ),
  ...
  ledger: (t) => (
    <g>
      ...
      <path {...accent(t)} d="m10.4 15.7 2.5-3 2.2 1.7 3-4.5" />
    </g>
  ),
}
...
interface ToolIconProps {
  name: ToolIconName
  /** Rendered size in px. Defaults to the 26px used on the tools index. */
  size?: number
  /** Explicit colours for contexts that cannot use CSS, such as OG images. */
  tone?: IconTone
  /** Stroke weight. Scale it up slightly on large renders so it stays true. */
  strokeWidth?: number
}
...
export default function ToolIcon({
  ...
  return (
    <svg
      className={tone ? undefined : 'tool-icon'}
      width={size}
      height={size}
      viewBox="0 0 24 24"
      fill="none"
      stroke={tone ? tone.base : 'currentColor'}
