...
export default function StrategyContent() {
  ...
  return (
    <main>
      ...
      <div className="strategy-progress-bar-mobile">
        <div style={{ width: `${scrollProgress * 100}%` }} className="strategy-progress-bar-fill" />
      </div>

      <div className="page-hero hero-animate-in">
      ...
      <section style={{ padding: '16px 20px 40px' }}>
        <div className="strategy-layout">
          ...
          <div className="strategy-sidebar">
            <div className="strategy-sidebar-line">
              <div
                className="strategy-sidebar-progress"
                style={{ height: `${((activeStep + 1) / strategies.length) * 100}%` }}
              />
            </div>
            {strategies.map((s, i) => (
              <button
                key={s.num}
                className={`strategy-sidebar-item${activeStep === i ? ' active' : ''}${activeStep > i ? ' done' : ''}`}
          ...
          <div className="container-prose">
            {strategies.map((s, i) => (
              ...
              >
                ...
                {s.body.map((p, j) => (
                  <p key={j} className="strategy-desc" style={{ marginBottom: j < s.body.length - 1 ? 14 : 0, whiteSpace: 'pre-line', lineHeight: 1.85, fontSize: 15 }}>{p}</p>
                ))}
                <div style={{
                  ...
                  <p style={{ fontSize: 13, color: 'var(--army)', fontWeight: 600, fontStyle: 'italic' }}>
                    📌 {s.callout}
                  </p>
                </div>
              </div>
